...
class TestCheckDeviceStatusUseCase:
    ...
    async def test_it_includes_timing_information(self, use_case, mock_device_gateway):
        device_ip = "192.168.1.100"
        ...
        sys_component = SystemComponent.from_raw_data(
            {
                ...
                "config": {"device": {"fw_id": "20230913-114010/v1.14.0-gcb84623"}},
            }
        )

        device_status = DeviceStatus(
            device_ip=device_ip, components=[sys_component], total_components=1
        )
        mock_device_gateway.get_device_status = AsyncMock(return_value=device_status)
