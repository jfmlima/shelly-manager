...
class TestShellyDevice:
    ...
    def test_it_creates_device_with_required_fields(self):
        device = ShellyDevice(ip="192.168.1.100", status=DeviceStatus.DETECTED)
    ...
    def test_it_validates_valid_ip_addresses(self):
        ...
        for ip in valid_ips:
            device = ShellyDevice(ip=ip, status=DeviceStatus.DETECTED)
            assert device.ip == ip
    ...
    def test_it_rejects_invalid_ip_addresses(self):
        ...
        for ip in invalid_ips:
            with pytest.raises(ValidationError):
                ShellyDevice(ip=ip, status=DeviceStatus.DETECTED)

    def test_it_validates_response_time_non_negative(self):
        device = ShellyDevice(
            ip="192.168.1.100", status=DeviceStatus.DETECTED, response_time=0.0
        )
    ...
    def test_it_rejects_negative_response_time(self):
        with pytest.raises(ValidationError):
            ShellyDevice(
                ip="192.168.1.100", status=DeviceStatus.DETECTED, response_time=-0.1
            )

    def test_it_accepts_all_device_statuses(self):
        for status in DeviceStatus:
            device = ShellyDevice(ip="192.168.1.100", status=status)
    ...
    def test_it_creates_device_with_minimal_required_data(self):
        device = ShellyDevice(ip="10.0.0.1", status=DeviceStatus.UNREACHABLE)

        assert device.ip == "10.0.0.1"
        assert device.status == DeviceStatus.UNREACHABLE

    def test_it_handles_auth_required_status(self):
        device = ShellyDevice(
            ip="192.168.1.100", status=DeviceStatus.AUTH_REQUIRED, auth_required=True
        )

        assert device.status == DeviceStatus.AUTH_REQUIRED
        assert device.auth_required is True

    def test_it_handles_device_with_error_message(self):
        error_msg = "Connection timeout after 5 seconds"
        device = ShellyDevice(
            ip="192.168.1.100", status=DeviceStatus.UNREACHABLE, error_message=error_msg
        )
    ...
    def test_it_creates_device_with_high_response_time(self):
        device = ShellyDevice(
            ip="192.168.1.100", status=DeviceStatus.DETECTED, response_time=5.0
        )
